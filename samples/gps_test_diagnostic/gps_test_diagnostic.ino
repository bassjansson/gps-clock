...
static long          baud_table[]       = {1200, 2400, 4800, 9600, 14400, 19200, 28800, 31250, 38400, 57600, 115200};
...
void displaySentences()
{
    ...
    dumpSomeChars();
    DEBUG_PORT << F("\nDevice baud rate is ") << baud_table[baud_index] << '\n';
    ...
    if ((ms_since_last_rx > 5) && waited_long_enough)
    {
        ...
        else if (valid_sentence_received)
        {
            ...
            if (ms < 10)
                DEBUG_PORT.print('0');
            DEBUG_PORT.print(ms);
            DEBUG_PORT.println(F(" seconds ago.\n"
...
static void GPSloop()
{
    while (gpsPort.available())
    {
        ...
        if (gps.decode(c) == NMEAGPS::DECODE_COMPLETED)
        {
            ...
            if (long_enough || ((sentences_printed++ >= 20) && (someCharsIndex >= MAX_SAMPLE)))
            {
                displaySentences();
                hang();
            }
    ...
    if (!valid_sentence_received || (millis() - valid_sentence_received > 3000UL))
        listenForSomething();
...
void setup()
{
    ...
    DEBUG_PORT.println(F("Looking for GPS device on RX/TX"));
...
#if !defined(NMEAGPS_PARSE_GGA) & !defined(NMEAGPS_PARSE_GLL) & !defined(NMEAGPS_PARSE_GSA) & !defined(NMEAGPS_PARSE_GSV) \
    & !defined(NMEAGPS_PARSE_RMC) & !defined(NMEAGPS_PARSE_VTG) & !defined(NMEAGPS_PARSE_ZDA) & !defined(NMEAGPS_PARSE_GST)
    warnings++;
    DEBUG_PORT.println(F("\nWarning: no messages are enabled for parsing in NMEAGPS_cfg.h.\n  No fields will be valid, "
                         "including the 'status' field."));
#endif
